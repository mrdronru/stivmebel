 ...
 * инлайн-разметку **полужирный**, *курсив*, `код`, [текст](ссылка), ![alt](путь).
 *
 ...
 *
 * 15.07.2026: добавлена поддержка ![alt](путь) — картинки внутри статей
 * Азбуки. Картинки статьи кладутся в abc/images/<slug>/ (см. PROJECT.md,
 * раздел «Азбука мебельщика»), путь в MD указывается относительно abc/,
 * например images/ergonomika-kuhni/01-vytyazhka.png.
 */
...
function inlineMarkdown(string $s): string {
    ...
    // Картинки — ОБЯЗАТЕЛЬНО до правила для ссылок ниже: у ![alt](путь) и
    // [текст](ссылка) общий "хвост" `(...)`, при обратном порядке regex
    // ссылки перехватит картинку раньше и превратит её в битую ссылку.
    $s = preg_replace('/!\[(.*?)\]\((.+?)\)/', '<img src="$2" alt="$1" loading="lazy">', $s);
