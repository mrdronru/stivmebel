...
USAGE = (
    ...
    "[--max-profiles N] [--out FILE.csv] [--headless=false] [--debug]\n"
...
while i < len(args):
    ...
    if arg.startswith("--max-profiles"):
        ...
        print("        Доступные флаги: --max-profiles N, --out FILE.csv, --headless=false, --debug")
        print(USAGE)
        sys.exit(1)
    else:
...
STALL_LIMIT = 3             # сколько подряд докруток без новой порции считаем "конец списка"
MAX_SCROLL_ITERS = 500      # аварийный потолок на число докруток, чтобы не зациклиться навечно
LOAD_WAIT_MAX = 10.0        # сколько максимум ждать новую порцию после докрутки (сайт "задумывается" на 5-8 сек)
LOAD_POLL_INTERVAL = 1.0    # с каким шагом опрашивать высоту страницы во время ожидания
...
def scroll_and_collect_links(page) -> set:
    ...
    while stall_count < STALL_LIMIT and iters < MAX_SCROLL_ITERS:
        ...
        # Ждём новую порцию, опрашивая высоту документа, а не фиксированную паузу —
        # сайт отвечает не сразу, а через 5-8 секунд.
        grew = False
        waited = 0.0
        ...
        if grew:
            stall_count = 0
            print(f"  подгрузилась новая порция (высота {last_height}px -> {new_height}px, шаг {iters}, ждали {waited:.0f}с)")
        else:
            stall_count += 1
            print(f"  [шаг {iters}] новых карточек не появилось за {waited:.0f}с (попытка {stall_count}/{STALL_LIMIT})")
        last_height = new_height

        # Если задан --max-profiles, не тратим время на докрутку до самого
        # конца списка (там может быть 1500+ профилей) — достаточно дешёвой
        # оценки числа карточек в DOM (без полного парсинга через BeautifulSoup,
        # он делается только один раз в самом конце).
        if MAX_PROFILES:
            try:
                rough_count = page.evaluate('document.querySelectorAll(\'a[href^="/pro/"]\').length')
            except Exception:
                rough_count = 0
            if rough_count >= MAX_PROFILES * 1.5:
                print(f"  карточек уже заметно больше --max-profiles ({rough_count}) — прекращаю прокрутку раньше конца списка")
                break
...
def main():
    with sync_playwright() as pw:
        ...
        print(f"\nБуду обходить {len(profile_urls)} профилей...\n")
        ...
        with open(OUT_PATH, "w", newline="", encoding="utf-8-sig") as f:
            ...
            for i, url in enumerate(profile_urls, 1):
                ...
                try:
                    ...
                    print(f"  [WARN] ошибка при обработке: {e}")
                    data = None
    ...
    # Подстраховка: настоящее портфолио уникально для каждого дизайнера.
    # Если одно и то же значение projects встретилось у ≥2 разных профилей —
    # это не личное портфолио, а какой-то общий виджет сайта, затесавшийся
    # в выборку по ошибке — чистим.
    from collections import Counter
    proj_counts = Counter(r["projects"] for r in results if r["projects"].strip())
    stripped = 0
    for r in results:
        if r["projects"].strip() and proj_counts[r["projects"]] > 1:
            ...
            stripped += 1
    if stripped:
        print(f"  [INFO] Обнаружены и очищены {stripped} значений 'projects', повторявшихся у разных профилей (похоже на чужой/общий блок сайта).")
        with open(OUT_PATH, "w", newline="", encoding="utf-8-sig") as f:
            writer = csv.DictWriter(f, fieldnames=[
                "name", "url", "city", "phone", "email", "website",
                "telegram", "whatsapp", "vk", "dzen", "youtube", "instagram", "other_social",
    ...
    print(f"\n✅ Готово. Сохранено профилей с контактами: {len(results)}. Пропущено без контактов: {skipped}.")
